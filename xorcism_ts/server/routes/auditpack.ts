...
import { generateAuditPackage, auditPackageMarkdown, auditPackageOscal, auditPackageSarif } from "../auditpack";

const router = Router();
const tenantOf = (req: Request): number | null => (req.user!.isSuperAdmin ? null : (req.user!.tenantId ?? null));

// GET /api/audit-package?format=json|md|oscal|poam|sarif — generate the audit & accreditation package
router.get("/audit-package", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (fmt === "sarif") {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="xorcism-findings.sarif.json"`);
      return void res.send(JSON.stringify(auditPackageSarif(pkg), null, 2));
    }
